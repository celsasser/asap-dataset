...
1. Is there a difference in average note length over time?
...
tbl_music_perfs |>
  ...
  mutate(
    year_middle = year_born + (year_died - year_born) / 2
  ) |>
  ggplot(mapping = aes(x = avg_note_duration, y = year_middle, color = composer)) +
  geom_jitter() +
  labs(
...
2. Is there a difference in average velocity over time?
...
tbl_music_perfs |>
  ...
  ggplot(mapping = aes(x = avg_velocity, y = year_middle, color = composer)) +
  geom_jitter() +
  labs(
    title = "Average Note Length by Composition",
    x = "Average Note Duration",
...
3. Is there a difference in average note length by performer?
```{r}
#| fig-width: 8
#| fig-height: 8
tbl_fugue_bwv_848 |>
...
4. Is there a difference in average note velocity by performer?
```{r}
#| fig-width: 8
#| fig-height: 8
tbl_fugue_bwv_848 |>
...
5. Is there a difference in note velocity variance by performer?
```{r}
#| fig-width: 8
#| fig-height: 8
tbl_fugue_bwv_848 |>
...
6. Is there a difference in note duration variance by performer?
```{r}
#| fig-width: 8
#| fig-height: 8
tbl_fugue_bwv_848 |>
...
7. Is there a difference in note duration variability over time?
